...
- [ ] **Install dev dependencies** — Add `typescript` (>=5.0), `vitest`, `eslint`, and `@types/node` as dev dependencies in `package.json`. Run `npm install` to generate `node_modules` and lock file. | Status: not_done
- [ ] **Configure Vitest** — Add a `vitest.config.ts` file (or configure within `package.json`) so that `npm run test` discovers and runs `src/__tests__/**/*.test.ts` files. | Status: not_done
- [ ] **Configure ESLint** — Add an ESLint config file appropriate for TypeScript. Ensure `npm run lint` works against `src/`. | Status: not_done
...
- [ ] **Verify build pipeline** — Run `npm run build` (tsc) and confirm it compiles successfully with the empty/skeleton source files and produces output in `dist/`. | Status: not_done

---

## Phase 2: Type Definitions (`src/types.ts`)

- [ ] **Define `ScratchpadEntry<T>` interface** — Include fields: `key` (string), `value` (T), `createdAt` (number), `updatedAt` (number), `accessedAt` (number), `ttl` (number | null), `slidingTtl` (boolean), `tags` (string[]). | Status: not_done
- [ ] **Define `EntryOptions` interface** — Include optional fields: `ttl` (number | null), `slidingTtl` (boolean), `tags` (string[]). | Status: not_done
...
- [ ] **Define `ScratchpadSnapshot` interface** — Include fields: `entries` (Record<string, ScratchpadEntry>), `timestamp` (number), `version` (literal 1). | Status: not_done
...
- [ ] **Define `ToContextOptions` interface** — Include fields: `format` ('markdown' | 'xml' | 'json' | 'kv'), `filterTags` (string[]), `filterNamespace` (string), `maxTokens` (number), `tokenCounter` ((text: string) => number), `includeMetadata` (boolean), `header` (string). | Status: not_done
- [ ] **Define `PersistenceAdapter` interface** — Include methods: `load(): Promise<ScratchpadSnapshot | null>`, `save(snapshot: ScratchpadSnapshot): Promise<void>`. | Status: not_done
- [ ] **Define `ScratchpadEvents` interface** — Define event payloads: `set` ({key, entry, isUpdate}), `delete` ({key, entry}), `expire` ({key, entry}), `clear` ({count}). Define `ScratchpadEventName` and `ScratchpadEventHandler<E>` types. | Status: not_done
...
- [ ] **Implement `ScratchpadError` base class** — Extend `Error` with a `readonly code: string` property. Set the error name to `'ScratchpadError'`. | Status: not_done
- [ ] **Implement `ScratchpadConfigError`** — Extend `ScratchpadError` with code `'SCRATCHPAD_CONFIG_ERROR'`. Used for invalid configuration in `createScratchpad()`. | Status: not_done
...
- [ ] **Implement `ScratchpadVersionError`** — Extend `ScratchpadError` with code `'SCRATCHPAD_VERSION_ERROR'` and a `readonly version: number` property. Used when a snapshot has an unsupported version. | Status: not_done

---

## Phase 4: TTL Logic (`src/ttl.ts`)

- [ ] **Implement `isExpired(entry, now)` function** — For fixed TTL: check if `now - entry.createdAt >= entry.ttl`. For sliding TTL: check if `now - entry.accessedAt >= entry.ttl`. Return `false` if `entry.ttl` is `null`. | Status: not_done
- [ ] **Implement `getExpiresAt(entry)` helper** — Calculate and return the absolute expiration timestamp for an entry based on its TTL mode. Return `null` if no TTL is set. Useful for debugging/stats. | Status: not_done
...
- [ ] **Implement `createScratchpad(options?)` factory function** — Accept `ScratchpadOptions`, validate configuration, create internal `Map<string, ScratchpadEntry>`, return a `Scratchpad` instance. | Status: not_done
...
- [ ] **Implement `set<T>(key, value, options?)`** — Create or update an entry in the internal Map. On create: set `createdAt`, `updatedAt`, `accessedAt` to `now()`. Inherit `defaultTtl` and `defaultSlidingTtl` if not overridden. On update: update `updatedAt`, `value`, and any provided options; preserve `createdAt`. Validate key is a non-empty string. | Status: not_done
- [ ] **Implement `get<T>(key)`** — Retrieve value by key. Check expiration first; if expired, delete entry, fire `expire` event, return `undefined`. If live, update `accessedAt` to `now()` and return the value. Return `undefined` if key does not exist. | Status: not_done
- [ ] **Implement `has(key)`** — Check if key exists and is not expired. If expired, remove entry and fire `expire` event, return `false`. | Status: not_done
- [ ] **Implement `delete(key)`** — Remove entry by key. If entry existed, fire `delete` event and return `true`. Otherwise return `false`. | Status: not_done
- [ ] **Implement `clear()`** — Remove all entries. Fire `clear` event with the count of removed entries. | Status: not_done
- [ ] **Implement `keys()`** — Return array of all non-expired keys. Trigger lazy expiration for any expired entries encountered during iteration. | Status: not_done
- [ ] **Implement `entries()`** — Return array of `[key, ScratchpadEntry]` tuples for all non-expired entries. Trigger lazy expiration for expired entries encountered. | Status: not_done
...
- [ ] **Handle `EntryOptions.ttl = null` to disable default TTL** — When caller passes `{ ttl: null }` explicitly, the entry should have no TTL even if the scratchpad has a `defaultTtl`. Distinguish between "ttl not provided" (inherit default) and "ttl explicitly set to null" (no expiration). | Status: not_done
...
- [ ] **Implement `findByTag(tag)`** — Iterate all non-expired entries, return entries whose `tags` array includes the given tag string (exact match). Trigger lazy expiration. | Status: not_done

### 5e: Stats

- [ ] **Implement `stats()`** — Return a `ScratchpadStats` object: compute `size` (non-expired), `rawSize` (total including expired-not-yet-evicted), `namespaceCount`, `namespaces` (distinct namespace prefixes), `entriesWithTtl`, `expiredPending`, `tagCounts` (count per tag), `oldestEntryAt`, `newestEntryAt`. | Status: not_done

---

## Phase 6: Event System

- [ ] **Implement internal event emitter** — Create a lightweight event emitter (no external dependencies) supporting the four event types: `set`, `delete`, `expire`, `clear`. Store handlers in a `Map<string, Set<Function>>`. | Status: not_done
- [ ] **Implement `on(event, handler)` method** — Register a handler for an event. Return an unsubscribe function that removes the handler when called. | Status: not_done
- [ ] **Fire `set` event** — When `set()` is called, fire with `{ key, entry, isUpdate }`. `isUpdate` is `true` when the key already existed. | Status: not_done
- [ ] **Fire `delete` event** — When `delete()` is called and the entry existed, fire with `{ key, entry }`. | Status: not_done
- [ ] **Fire `expire` event** — When an entry is lazily or actively evicted due to TTL, fire with `{ key, entry }`. | Status: not_done
- [ ] **Fire `clear` event** — When `clear()` is called, fire with `{ count }` (number of entries removed). | Status: not_done
- [ ] **Unsubscription function** — Verify that calling the returned unsubscribe function prevents the handler from being called on subsequent events. | Status: not_done

---

## Phase 7: Namespace Implementation (`src/namespace.ts`)

- [ ] **Implement `namespace(name)` method** — Return a `Scratchpad`-compatible wrapper that prefixes all keys with `name:`. The wrapper delegates all operations to the parent scratchpad's internal Map with key transformation. | Status: not_done
- [ ] **Validate namespace name** — Must be a non-empty string without `:` characters. Throw an error if invalid. | Status: not_done
- [ ] **Implement key prefixing on `set`, `get`, `has`, `delete`, `getEntry`** — All single-key operations prepend the namespace prefix. | Status: not_done
- [ ] **Implement `keys()` on namespace** — Return only keys starting with the namespace prefix, with the prefix stripped from each key. | Status: not_done
- [ ] **Implement `entries()` on namespace** — Return only entries with the namespace prefix, stripping the prefix from keys. | Status: not_done
- [ ] **Implement `clear()` on namespace** — Delete only entries with the namespace prefix. Fire `clear` event with count of removed entries. | Status: not_done
- [ ] **Implement `size` on namespace** — Count only non-expired entries within the namespace prefix. | Status: not_done
- [ ] **Implement `findByTag()` and `find()` on namespace** — Search only within entries belonging to the namespace. | Status: not_done
- [ ] **Implement nested namespaces** — `namespace(name)` on a namespace wrapper concatenates prefixes: `outer:inner:`. Support arbitrary nesting depth. | Status: not_done
- [ ] **Implement `toContext()` on namespace** — Render only entries within the namespace. | Status: not_done
- [ ] **Implement `snapshot()` and `restore()` on namespace** — Snapshot/restore scoped to the namespace entries (or delegate to the parent with appropriate filtering). | Status: not_done
- [ ] **Namespace event key format** — Events fired from namespace operations include the full key (with prefix) so the root scratchpad's event listeners see the full key path. | Status: not_done
- [ ] **Cross-namespace access** — Verify that the root scratchpad can access any entry by its full key including namespace prefix (e.g., `pad.get("agent1:result")`). | Status: not_done

---

## Phase 8: Snapshot and Diff Implementation (`src/snapshot.ts`)

- [ ] **Implement `snapshot()` method** — Deep-copy all entries using `JSON.parse(JSON.stringify())`. Return a `ScratchpadSnapshot` with `entries`, `timestamp` (current `now()`), and `version: 1`. The snapshot must be immutable — mutations to the scratchpad after snapshot do not affect it. | Status: not_done
- [ ] **Implement `restore(snapshot)` method** — Replace the scratchpad's entire state with the snapshot's entries. Fire `clear` event for the existing entries, then fire `set` events for each restored entry. | Status: not_done
- [ ] **Validate snapshot version on restore** — Throw `ScratchpadVersionError` if the snapshot's `version` field is not `1`. | Status: not_done
- [ ] **Implement `serialize()` method** — Equivalent to `snapshot()` but explicitly guarantees JSON-serializability. Return a `ScratchpadSnapshot` suitable for `JSON.stringify()`. | Status: not_done
- [ ] **Implement `fromSnapshot(snapshot, options?)` static function** — Create a new scratchpad instance and restore the snapshot into it. Accept optional `ScratchpadOptions` for configuration. Validate snapshot version. | Status: not_done
...
- [ ] **Implement Markdown format** — Render entries as a Markdown section with `## Working Memory` header (or custom header) and `- **key**: value` bullet points. Values should be JSON-stringified if not strings. | Status: not_done
- [ ] **Implement XML format** — Render entries as `<working-memory><entry key="...">value</entry>...</working-memory>`. Values should be JSON-stringified if not strings. | Status: not_done
- [ ] **Implement JSON format** — Render entries as a JSON object `{ "key1": value1, "key2": value2, ... }`. | Status: not_done
- [ ] **Implement key-value (kv) format** — Render entries as `key: value` pairs, one per line. Values should be JSON-stringified if not strings. | Status: not_done

### 9b: Filtering

- [ ] **Implement `filterTags` option** — When provided, only include entries that have at least one of the specified tags (OR logic). | Status: not_done
- [ ] **Implement `filterNamespace` option** — When provided, only include entries within the specified namespace prefix. | Status: not_done
- [ ] **Implement combined filtering** — When both `filterTags` and `filterNamespace` are provided, entries must satisfy both conditions (AND logic). | Status: not_done

### 9c: Token Budget

- [ ] **Implement `maxTokens` truncation** — Render entries from newest to oldest (by `updatedAt`). Accumulate token count using the `tokenCounter` function. Stop rendering when the next entry would exceed the budget. | Status: not_done
...
- [ ] **Handle empty scratchpad** — Return an empty/minimal string when there are no entries to render (empty Markdown section, empty XML tags, empty JSON object, empty string for kv). | Status: not_done

### 9f: Custom Header

- [ ] **Implement `header` option** — Allow customizing the header text. Default is `'Working Memory'` for Markdown format, undefined for others. | Status: not_done
...
- [ ] **Implement `pad.save()` method** — Serialize current state and call `adapter.save()`. No-op if no persistence adapter is configured. Throw `ScratchpadPersistenceError` if save fails. | Status: not_done
- [ ] **Implement `pad.load()` method** — Call `adapter.load()`. If a snapshot is returned, restore it (replacing current state). No-op if no adapter is configured. Throw `ScratchpadPersistenceError` if load fails. Throw `ScratchpadVersionError` if loaded snapshot has unsupported version. | Status: not_done
...
- [ ] **Fire `expire` events during sweep** — Each expired entry removed by the sweep should trigger an `expire` event. | Status: not_done
- [ ] **Clear sweep interval on `destroy()`** — Stop the interval timer when the scratchpad is destroyed. | Status: not_done

---

## Phase 12: Lifecycle (`destroy()`)

- [ ] **Implement `destroy()` method** — Clear the sweep interval (if active). Flush any pending debounced auto-save (call `adapter.save()` immediately). Remove all event handlers. Return a `Promise<void>`. | Status: not_done
...
- [ ] **Export `createScratchpad`** — The primary factory function. | Status: not_done
- [ ] **Export `fromSnapshot`** — Static function to create a scratchpad from a serialized snapshot. | Status: not_done
...
- [ ] **Test `set()` and `get()` with string values** — Verify round-trip of string values. | Status: not_done
...
- [ ] **Test `get()` returns `undefined` for missing keys** — Verify non-existent key returns undefined. | Status: not_done
- [ ] **Test `has()` returns true for existing keys** — Verify `has()` correctness. | Status: not_done
- [ ] **Test `has()` returns false for missing keys** — Verify `has()` with non-existent key. | Status: not_done
- [ ] **Test `delete()` removes entries** — Set a key, delete it, verify `get()` returns undefined. | Status: not_done
- [ ] **Test `delete()` returns true when entry existed** — Verify return value. | Status: not_done
- [ ] **Test `delete()` returns false when entry did not exist** — Verify return value for non-existent key. | Status: not_done
- [ ] **Test `clear()` removes all entries** — Set multiple entries, call `clear()`, verify all are gone. | Status: not_done
...
- [ ] **Test update behavior** — Call `set()` twice with the same key. Verify `value` is updated, `updatedAt` changes, `createdAt` is preserved. | Status: not_done
...
- [ ] **Test fixed TTL: entry accessible before expiry** — Use mock `now()`, set entry with TTL, advance time less than TTL, verify `get()` returns value. | Status: not_done
- [ ] **Test fixed TTL: entry expired after TTL elapses** — Advance time past TTL, verify `get()` returns `undefined`. | Status: not_done
- [ ] **Test fixed TTL: accessing does not extend lifetime** — Set entry with fixed TTL, call `get()` partway through, advance past original TTL, verify expiration. | Status: not_done
- [ ] **Test sliding TTL: entry stays alive with access** — Set entry with sliding TTL, call `get()` before TTL elapses, verify TTL resets. Advance past original TTL but not past last access + TTL, verify still alive. | Status: not_done
- [ ] **Test sliding TTL: entry expires when not accessed** — Set entry with sliding TTL, do not access it, advance past TTL, verify expiration. | Status: not_done
- [ ] **Test default TTL inheritance** — Create scratchpad with `defaultTtl`, set entries without specifying TTL, verify they expire after the default duration. | Status: not_done
...
- [ ] **Test explicit `null` TTL disables default** — Create scratchpad with `defaultTtl`, set entry with `{ ttl: null }`, verify the entry never expires. | Status: not_done
...
- [ ] **Test `findByTag()` excludes expired entries** — Verify lazy expiration during `findByTag()`. | Status: not_done
- [ ] **Test `expire` event fires on lazy expiration** — Register an `expire` handler, access an expired entry, verify the handler is called with the correct payload. | Status: not_done
...
- [ ] **Test `findByTag()` returns matching entries** — Set entries with various tags, call `findByTag("tool-result")`, verify correct entries are returned. | Status: not_done
- [ ] **Test `findByTag()` with no matches** — Call `findByTag()` with a tag that no entry has, verify empty array is returned. | Status: not_done
...
- [ ] **Test `set` event fires on create** — Register handler, call `set()`, verify handler called with `isUpdate: false`. | Status: not_done
- [ ] **Test `set` event fires on update** — Set a key, register handler, call `set()` again on same key, verify handler called with `isUpdate: true`. | Status: not_done
- [ ] **Test `delete` event fires** — Register handler, set a key, delete it, verify handler called with the entry. | Status: not_done
...
- [ ] **Test `expire` event fires** — Register handler, set entry with TTL, advance time past TTL, access the entry, verify handler is called. | Status: not_done
- [ ] **Test `clear` event fires** — Register handler, set entries, call `clear()`, verify handler called with correct count. | Status: not_done
- [ ] **Test unsubscribe function** — Register handler, get unsubscribe function, call it, fire event, verify handler is NOT called. | Status: not_done
...
- [ ] **Test namespace creation and key prefixing** — Create namespace, set a key, verify it is stored as `namespace:key` in the root. | Status: not_done
- [ ] **Test namespace `get()` reads prefixed key** — Set via namespace, get via namespace, verify correct value. | Status: not_done
- [ ] **Test namespace isolation** — Set same key in two different namespaces, verify they do not conflict. | Status: not_done
- [ ] **Test namespace `keys()` returns only namespace keys** — Set entries in root and namespace, verify namespace `keys()` only returns namespace entries. | Status: not_done
- [ ] **Test namespace `keys()` strips prefix** — Verify keys returned by `ns.keys()` do not include the namespace prefix. | Status: not_done
...
- [ ] **Test namespace `has()` and `delete()`** — Verify they operate only on prefixed keys. | Status: not_done
- [ ] **Test namespace `findByTag()`** — Verify it searches only within the namespace. | Status: not_done
- [ ] **Test nested namespaces** — Create `pad.namespace("a").namespace("b")`, set a key, verify stored as `a:b:key` in root. | Status: not_done
- [ ] **Test cross-namespace access from root** — Set via `pad.namespace("agent1")`, read via `pad.get("agent1:key")`, verify it works. | Status: not_done
...
- [ ] **Test snapshot captures all entries** — Set entries, take snapshot, verify snapshot contains all entries with correct values and metadata. | Status: not_done
- [ ] **Test snapshot is a deep copy** — Take snapshot, modify scratchpad, verify snapshot is unchanged. | Status: not_done
- [ ] **Test restore replaces state** — Set entries, snapshot, modify, restore, verify state matches snapshot. | Status: not_done
- [ ] **Test restore fires events** — Verify `clear` event followed by `set` events for each restored entry. | Status: not_done
- [ ] **Test restore with version validation** — Attempt to restore a snapshot with `version: 2`, verify `ScratchpadVersionError` is thrown. | Status: not_done
...
- [ ] **Test `fromSnapshot()` creates new scratchpad from snapshot** — Take snapshot, create new scratchpad with `fromSnapshot()`, verify entries match. | Status: not_done
...
- [ ] **Test `pad.save()` calls adapter** — Call `save()` explicitly, verify `adapter.save()` is called with correct snapshot. | Status: not_done
- [ ] **Test `pad.load()` replaces state** — Load into a non-empty scratchpad, verify state is replaced with loaded data. | Status: not_done
...
- [ ] **Test save/load are no-ops without adapter** — Create scratchpad without persistence, call `save()` and `load()`, verify no errors. | Status: not_done

---

## Phase 21: Integration Tests — Context Rendering

- [ ] **Test Markdown format output** — Create entries, render with `format: 'markdown'`, verify output matches expected Markdown structure with header and bullet points. | Status: not_done
- [ ] **Test XML format output** — Create entries, render with `format: 'xml'`, verify output matches expected XML structure. | Status: not_done
- [ ] **Test JSON format output** — Create entries, render with `format: 'json'`, verify output is valid JSON with correct key-value pairs. | Status: not_done
- [ ] **Test key-value format output** — Create entries, render with `format: 'kv'`, verify output has `key: value` lines. | Status: not_done
- [ ] **Test default format is Markdown** — Call `toContext()` without specifying format, verify Markdown output. | Status: not_done
- [ ] **Test `filterTags` filtering** — Create entries with different tags, render with `filterTags`, verify only matching entries appear. | Status: not_done
- [ ] **Test `filterNamespace` filtering** — Create entries in different namespaces, render with `filterNamespace`, verify only matching entries appear. | Status: not_done
...
- [ ] **Test `maxTokens` truncation** — Create many entries, render with a small `maxTokens` and a `tokenCounter`, verify output is truncated and newest entries are prioritized. | Status: not_done
...
- [ ] **Test custom header** — Render with `header: 'Agent State'`, verify the custom header appears instead of default. | Status: not_done
...
- [ ] **Test sweep removes expired entries** — Configure `sweepIntervalMs`, set entries with TTL, advance mock time past TTL, trigger sweep, verify entries are removed. | Status: not_done
- [ ] **Test sweep fires `expire` events** — Register `expire` handler, trigger sweep that removes expired entries, verify events fire. | Status: not_done
...
- [ ] **Test sweep interval is cleared on `destroy()`** — Configure sweep, destroy the scratchpad, verify `setInterval` is cleared (no more sweeps occur). | Status: not_done
...
- [ ] **Test `stats()` returns correct statistics** — Create entries with various tags, TTLs, namespaces. Call `stats()` and verify all fields: `size`, `rawSize`, `namespaceCount`, `namespaces`, `entriesWithTtl`, `expiredPending`, `tagCounts`, `oldestEntryAt`, `newestEntryAt`. | Status: not_done
...
- [ ] **Verify `npm run build` succeeds** — Ensure `tsc` compiles all source files to `dist/` with declarations and source maps. | Status: not_done
...
- [ ] **Verify `dist/` output** — Confirm `dist/index.js`, `dist/index.d.ts` exist and export the expected public API. | Status: not_done
- [ ] **Verify `package.json` metadata** — Confirm `main`, `types`, `files`, `engines`, `publishConfig`, and all scripts are correct. | Status: not_done
...
- [ ] **Verify `prepublishOnly` script** — Confirm that `npm publish` triggers `npm run build` via the existing `prepublishOnly` script. | Status: not_done
